...
export default class GroupConfig extends React.Component {
  ...
  render() {
    ...
    // Caution Zone
    let cautionZone = [];
    cautionZone.push(<Divider className={'caution-zone'} orientation="center" plain>Caution Zone</Divider>);
    // Leave Group (Member) (Before Grouping DDL)
    if (isMember && !afterGroupingDDL) {
      cautionZone.push(<Button danger block size={'large'}>Leave Group</Button>);
    }
    // Full (Group Owner) (Before Grouping DDL) (Available when Member Limits Reached)
    if (isOwner && !afterGroupingDDL) {
      cautionZone.push(
        <Button danger block size={'large'}>{isFull ? `Unmark Group as Full` : `Mark Group as Full`}</Button>
      );
    }
    // Submit (Group Owner) (After Grouping DDL) (Before Approve)
    if (isOwner && afterGroupingDDL && !isApproved) {
      cautionZone.push(
        <Button danger block size={'large'}>{isSubmitted ? 'Revert Submission' : 'Submit Project for Approval'}</Button>
      );
    }
    // Approve (Admin) (After Proposal DDL) (Submitted)
    if (isAdmin && afterProposalDDL && isSubmitted) {
      cautionZone.push(<Button danger block size={'large'}>Approve Proposal</Button>);
    }
    // Transfer Ownership (Group Owner or Admin) (All stages)
    if (isOwner || isAdmin) {
      cautionZone.push(<Button danger block size={'large'}>Transfer Group Owner</Button>);
    }
    ...
    if ((isOwner && !afterGroupingDDL) || (isAdmin && !afterProposalDDL)) {
      cautionZone.push(<Button danger block size={'large'}>Dismiss Group</Button>);
    }
